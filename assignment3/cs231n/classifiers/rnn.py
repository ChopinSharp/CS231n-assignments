...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # forward
        out, rnn_cache = forward(x, h0, Wx, Wh, b)
        ...
        dx, dh0, grads['Wx'], grads['Wh'], grads['b'] = backward(dout, rnn_cache)
        grads['W_embed'] = word_embedding_backward(dx, embed_cache)
        grads['W_proj'] = features.transpose() @ dh0
    ...
    def sample(self, features, max_length=30):
        ...
        x = np.vstack((W_embed[self._start], ) * N)
        if self.cell_type == 'rnn':
            for i in range(max_length):
                h, _ = rnn_step_forward(x, h, Wx, Wh, b)
                captions[:, i] = np.argmax(h @ W_vocab + b_vocab, axis=1)
